...
module.exports = {
    ...
    async execute(client, interaction) {
        ...
        collector.on('collect', async collected => {
            ...
            let winner = 0;
            if(playerChoice == 0 && botChoice == 5 || playerChoice == 2 && botChoice == 0 || playerChoice == 5 && botChoice == 2)
            winner = 2;
            if(playerChoice == 5 && botChoice == 0 || playerChoice == 0 && botChoice == 2 || playerChoice == 2 && botChoice == 5)
            winner = 1;
            ...
            //如果沒有資料就創建一個新的並回覆結果
            if (found == false) {
                bank.push({ id: interaction.user.id, money: 500 });
                const resultEmbed = new EmbedBuilder()
                    .setColor('#5865F2')
                    .setTitle('剪刀石頭布！')
                    .setDescription(`結果：${earnings}元\n你現在有 ${500 + earnings} 元!`);
